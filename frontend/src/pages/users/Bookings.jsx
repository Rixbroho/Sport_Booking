...
import { Search, Filter, MapPin, Calendar, Clock, Users, Trash2, Edit2, CheckCircle, AlertCircle } from 'lucide-react';
...
const Bookings = ({ user, onLogout, setCurrentPage }) => {
  ...
  const filteredBookings = bookings.filter(booking => {
    ...
    const matchesSearch = booking.turf.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          {filteredBookings.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {filteredBookings.map((booking) => (
                <div key={booking.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition-all overflow-hidden">
                  {/* Status Bar */}
                  <div className={`h-1 ${booking.status === 'Confirmed' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                  
                  <div className="p-6">
                    {/* Header */}
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-xs font-mono text-gray-400 mb-1">{booking.id}</p>
                        <h3 className="text-xl font-bold text-gray-800">{booking.turf}</h3>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div className="flex items-center gap-2 text-gray-700">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-sm">{booking.location}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 text-gray-700">
            ...
            <div className="flex flex-col items-center justify-center h-96 text-center">
              ...
              <p className="text-gray-500 mb-6">You don't have any bookings matching your filters.</p>
