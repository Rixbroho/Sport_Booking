...
const UserBookingSetting = () => {
  ...
  // Determine admin status robustly: prefer `user` object,  back to decoding the JWT token payload.
      ...
      <div className="flex gap-2 border-b mb-6 overflow-x-auto">
        {["all", "pending", "confirmed", "declined"].map((status) => (
          ...
          >
            ...
            {status === "pending" && `Pending (${bookings.filter((b) => b.status === "Pending").length})`}
            {status === "confirmed" && `Confirmed (${bookings.filter((b) => b.status === "Confirmed").length})`}
            {status === "declined" && `Declined (${bookings.filter((b) => b.status === "Declined").length})`}
          </button>
        ))}
      </div>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">
                      Booked by: <span className="font-semibold">{booking.userName}</span>
                    </p>
                  </div>
                  <div
